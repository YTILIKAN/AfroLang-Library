...
export function DatasetCard({ dataset }: DatasetCardProps) {
  ...
  return (
    <article className="group border-l-2 border-hairline pl-5 transition hover:border-terracotta/60 hover:bg-savanna/40">
      <div className="border-t border-hairline pt-6 group-hover:border-terracotta/20">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 flex-1 space-y-2">
            <Link
              href={`/datasets/${dataset.id}`}
              className="font-display text-lg font-medium text-ink-black hover:text-indigo-deep"
            >
              {dataset.title}
            </Link>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <Link
            href={`/datasets/${dataset.id}`}
            className="font-mono-ui text-[11px] font-medium uppercase tracking-[0.012em] text-indigo-deep hover:text-terracotta"
          >
            Voir la fiche →
          </Link>
          <a
